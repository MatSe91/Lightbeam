...
/// <summary>
/// 3x3 Matrix für Unity. Credits an Scribe von: http://answers.unity3d.com/questions/695948/affine-transformations-to-object-2.html
/// </summary>
public class Matrix3x3
{
    ...
    /// <summary>
    /// Rotationsmatrix um den Winkel beta -> muss mit einem Vektor multipliziert werden.<para />
    /// int = 0 ==> Rotation um x<para />
    /// int = 1 ==> Rotation um y<para />
    /// int = 2 ==> Rotation um z<para />
    /// </summary>
    /// <param name="beta"></param>
    /// <param name="xyz"></param>
    public Matrix3x3(float beta, int xyz)
    {
        if (xyz == 0)
        {
            matrix = new float[3, 3];
            ...
            matrix[2, 0] = 0;
            matrix[0, 1] = 0;
            matrix[1, 1] = Mathf.Cos(beta * Mathf.Deg2Rad);
            matrix[2, 1] = -Mathf.Sin(beta * Mathf.Deg2Rad);
            matrix[0, 2] = 0;
            matrix[1, 2] = Mathf.Sin(beta * Mathf.Deg2Rad);
            matrix[2, 2] = Mathf.Cos(beta * Mathf.Deg2Rad);
        }
        else if (xyz == 1)
        {
            matrix = new float[3, 3];
            matrix[0, 0] = Mathf.Cos(beta * Mathf.Deg2Rad);
            matrix[1, 0] = 0;
            matrix[2, 0] = Mathf.Sin(beta * Mathf.Deg2Rad);
            matrix[0, 1] = 0;
            matrix[1, 1] = 1;
            matrix[2, 1] = 0;
            matrix[0, 2] = -Mathf.Sin(beta * Mathf.Deg2Rad);
            matrix[1, 2] = 0;
            matrix[2, 2] = Mathf.Cos(beta * Mathf.Deg2Rad);
        }
        else if (xyz == 2)
        {
            matrix = new float[3, 3];
            matrix[0, 0] = Mathf.Cos(beta * Mathf.Deg2Rad);
            matrix[1, 0] = -Mathf.Sin(beta * Mathf.Deg2Rad);
            matrix[2, 0] = 0;
            matrix[0, 1] = Mathf.Sin(beta * Mathf.Deg2Rad);
            matrix[1, 1] = Mathf.Cos(beta * Mathf.Deg2Rad);
            matrix[2, 1] = 0;
            matrix[0, 2] = 0;
            matrix[1, 2] = 0;
